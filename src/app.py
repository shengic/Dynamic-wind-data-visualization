...
Version: 2.5 (Tail + Arrow Head Rotate Together - Cleaned)
"""
...
def main():
    df = load_data()
    st.title("🌬️ 專業垂直風場動態演繹 (尾巴與箭頭同步旋轉版)")
    ...
    arrow_scale = st.sidebar.slider("箭頭整體大小", 8.0, 35.0, 18.0, step=0.5)
    ...
    def get_traces(data_slice):
        ...
        for _, row in sample.iterrows():
            ...
            # Use the SAME angle for tail and head
            theta = np.deg2rad(direction - 90)        # ← Change to +90 if direction is reversed

            length = arrow_scale * 0.25
    ...
    st.plotly_chart(fig, width="stretch", key="wind_arrow_v25_fixed")
